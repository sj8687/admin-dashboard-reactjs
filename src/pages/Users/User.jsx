...
export default function Users() {
    ...
    const { users, usersLoading } = useSelector((state) => state.posts);
    const [search, setSearch] = useState("");
    const [roleFilter, setRoleFilter] = useState("all");
    useEffect(() => {
        dispatch(fetchUsersRequest());
    }, [dispatch]);
    const filtered = useMemo(() => users.filter((u) => (u.name.toLowerCase().includes(search.toLowerCase()) ||
        u.email.toLowerCase().includes(search.toLowerCase())) &&
        (roleFilter === "all" || u.role === roleFilter)), [users, search, roleFilter]);
    return (<div className="space-y-6 px-4 sm:px-6 lg:px-8 py-6 bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users..." className="w-full lg:flex-1 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-900 dark:text-slate-200 placeholder-gray-400 dark:placeholder-slate-500 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition"/>

        <div className="flex flex-wrap gap-2">
          {["all", "admin", "manager", "customer"].map((r) => (<button key={r} onClick={() => setRoleFilter(r)} className={`px-3 py-2 rounded-xl text-xs font-semibold capitalize transition ${roleFilter === r
      ...
      <Card className="overflow-hidden">
        {usersLoading ? (<div className="py-20 text-center text-gray-400 dark:text-slate-400">
            Loading users...
          </div>) : (<>
            {/* Desktop Table */}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-slate-700">
                    {["User", "Email", "Phone", "Role", "Orders", "City", "Actions"].map((h) => (<th key={h} className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">
                        {h}
                      </th>))}
                  </tr>
                </thead>
                ...
                <tbody>
                  {filtered.map((u) => (<tr key={u.id} className="border-b border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-800 transition">
            ...
            <div className="md:hidden p-4 space-y-4">
              {filtered.map((u) => (<div key={u.id} className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 space-y-3">
                  <div className="flex items-center gap-3">
                    <Avatar initials={u.avatar} size="w-8 h-8" text="text-xs"/>
                    <div>
            ...
            {filtered.length === 0 && (<div className="text-center py-12 text-gray-500 dark:text-slate-400">
                No users found
              </div>)}
          </>)}
      </Card>
